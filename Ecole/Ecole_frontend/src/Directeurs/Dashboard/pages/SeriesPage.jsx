/**
 * SeriesPage — Gestion des séries (CRUD)
 *
 * Composant autonome avec modal pour création/édition/suppression.
 */
...
export default function SeriesPage({ series, onRefresh }) {
  ...
  const handleEdit = (serie) => {
    ...
    setFormData({ nom: serie.nom });
    setIsModalOpen(true);
  };

  const handleDelete = async (id) => {
    if (!window.confirm('Êtes-vous sûr de vouloir supprimer cette série ?')) return;
    setLoading(true);
    try {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingSerie) {
        await api.put(`/series/${editingSerie.id}`, formData);
  ...
  return (
    <div className="series-page">
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ background: '#f8f9fa', textAlign: 'left' }}>
              <th style={{ padding: '1rem' }}>Nom de la série</th>
              <th style={{ padding: '1rem', textAlign: 'right' }}>Actions</th>
            </tr>
          </thead>
          <tbody>
            {series.length === 0 ? (
              <tr><td colSpan="2" style={{ padding: '1rem', textAlign: 'center' }}>Aucune série trouvée</td></tr>
            ) : (
              series.map(serie => (
                <tr key={serie.id} style={{ borderBottom: '1px solid #eee' }}>
                  <td style={{ padding: '1rem' }}>{serie.nom}</td>
                  <td style={{ padding: '1rem', textAlign: 'right' }}>
                    <button onClick={() => handleEdit(serie)} className="btn-icon" style={{ marginRight: '0.5rem', color: '#3b82f6' }}>
                      <Edit2 size={18} />
                    </button>
                    <button onClick={() => handleDelete(serie.id)} className="btn-icon" style={{ color: '#ef4444' }}>
                      <Trash2 size={18} />
                    </button>
                  </td>
                </tr>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingSerie ? 'Modifier la série' : 'Ajouter une série'}>
        ...
        <form onSubmit={handleSubmit}>
          <div className="form-group" style={{ marginBottom: '1.5rem' }}>
            <label style={{ display: 'block', marginBottom: '0.5rem' }}>Nom de la série</label>
            <input
