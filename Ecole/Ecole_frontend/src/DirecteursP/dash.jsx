    ...
    <div className="dashboard-container">
      ...
      <div className="main-content">
        ...
        <main className="content-area">
          ...
          {(activeTab !== 'aperçu' && activeTab !== 'élèves' && activeTab !== 'classes' && activeTab !== 'matieres' && activeTab !== 'LiaisonSeriesClass'  && activeTab !== 'LiaisonMatieresAvecCoefficientEtSerieClasses' && activeTab !== 'notes' && activeTab !== 'enseignantsauxclasses' && activeTab !== 'emploi' && activeTab !== 'Types' && activeTab !== 'LiaisonClassesPeriodeTypesEvaluation' && activeTab !== 'profil') && (
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const classesRes = await api.get(`/classesP?with_series=true&with_matieres=true`);
        await new Promise(resolve => setTimeout(resolve, 100));
        
        const classesMatieresRes = await api.get(`/with-series-matieresPrimaire`);
        await new Promise(resolve => setTimeout(resolve, 100));
...
const TypeEvas2 = () => {
  ...
  const AjouterPeriode = async () => {
    ...
    try {
      ...
      const errorMessage = err.response?.data?.message || "Erreur lors de l'ajout";
      console.error('Error adding periode:', err);
      //setError(errorMessage);
    } finally {
      setLoading(false);
    }
