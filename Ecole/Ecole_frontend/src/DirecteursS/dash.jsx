...
const fetchElevesParClasseMaternelle = async () => {
  try{
    ...
    setElevesSecondaire(res.data);
    console.log("Élèves par classe de secondaire:", res.data);
  } catch (err) {
    setElevesSecondaire([]);
    console.error("Erreur lors de la récupération des élèves par classe pour le secondaire:", err);
    ...
    <div className="dashboard-container">
      ...
      <div className="main-content">
        ...
        <main className="content-area">
          ...
          {activeTab === 'élèves' && (
            <div className="eleves-container">  
              ...
              <div className="eleves-list">
                ...
                ) : !Array.isArray(elevesSecondaire.data) ? (
                  <div className="empty-state error">
                    Erreur: Impossible de charger les élèves
                  </div>
                ) : elevesSecondaire.data.length === 0 ? (
                  <div className="empty-state">
                    Aucun élève trouvé. Ajoutez-en un ci-dessus.
                  </div>
                ) : (
                  // Regroupement par classe
                  Object.entries(groupElevesByClasse(elevesSecondaire.data)).map(([classeNom, eleves]) => (
          ...
          {(activeTab !== 'aperçu' && activeTab !== 'élèves' && activeTab !== 'classes' && activeTab !== 'matieres' && activeTab !== 'LiaisonSeriesClass'  && activeTab !== 'LiaisonMatieresAvecCoefficientEtSerieClasses' && activeTab !== 'notes' && activeTab !== 'enseignantsauxclasses'&& activeTab !== 'Types' && activeTab !== 'Contributions' && activeTab !== 'emploi' && activeTab !== 'liaisonClassesPeriodesTypes' && activeTab !== 'Fichier' && activeTab !== 'profil') && (
...
const LierEnseignantsAuxMatieres = () => {
  ...
  /*useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log("voci les erreurs:", classesRes)
        setMessage({ text: '', type: '' });
      } catch (error) {
  ...
  const fetchData = async () => {
    try {
      ...
      const filteredClassesData = classesRes.data.map(classe => ({
        ...classe,
        // Filtrer les séries uniques en utilisant un Set
        series: Array.from(new Set(
          classe.series?.map(serie => JSON.stringify({
            id: serie.id,
            nom: serie.nom,
            matieres: serie.matieres
          }))
        )).map(str => {
          const serie = JSON.parse(str);
          return {
            ...serie,
            matieres: serie.matieres?.map(matiere => ({
              ...matiere,
              enseignants: (matiere.enseignants || []).filter(enseignant => 
                enseignant.matiere_id === matiere.id &&
                enseignant.pivot?.classe_id === classe.id &&
                enseignant.pivot?.serie_id === serie.id
              )
            }))
          };
        })
      ...
      setClassesWithData(filteredClassesData);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        // Filtrer les données comme dans useEffect
        const filteredClassesData = classesRes.data.map(classe => ({
          ...classe,
  ...
  /*const getEnseignantsForMatiere = (matiereId) => {
    return enseignants.filter(enseignant => 
      Array.isArray(enseignant.matieres) && enseignant.matieres.some(m => m.id == matiereId)
    );
  };*/
  ...
  const getEnseignantsForMatiere = (matiereId) => {
    return enseignants.filter(enseignant => 
      enseignant.matiere_id === matiereId && 
      (!enseignant.pivot || 
        (enseignant.pivot.classe_id === parseInt(selectedClass) && 
        enseignant.pivot.serie_id === parseInt(selectedSerie)))
    );
  };
              ...
              <div className="form-group">
                ...
                <div className="matieres-enseignants-list">
                  {selectedMatieres.map(matiereId => {
                    const matiere = matieres.find(m => m.id == matiereId);
                    const enseignantsMatiere = getEnseignantsForMatiere(matiereId);
                    console.log("Enseignants pour la matière", matiereId, enseignantsMatiere);
                    const selectedEnseignants = enseignantsParMatiere[matiereId] || [];
