    ...
    public function initiatePayment(Request $request)
{
    ...
    try {
        ...
        // Persist transaction
        \App\Models\TransactionPaiement::create([
...
{
    try {
        $cp = $this->cinetPayService;

        if (!$cp->verifySignature($request->all())) {
        ...
        $tx = \App\Models\TransactionPaiement::where('reference_transaction', $request->cpm_trans_id)->first();
        if (!$tx) {
            return response('TX not found', 404);
        }
...
{
    $tx = \App\Models\TransactionPaiement::where('reference_transaction', $request->transaction_id)->first();
    if (!$tx) {
        return response()->json(['success' => false, 'message' => 'Transaction introuvable']);
    }
